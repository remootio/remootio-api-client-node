...
class RemootioDevice extends EventEmitter{
     ...
     * @param {string} [sendPingMessageEveryXMs=60000] - the API client sends a ping frame to the Remootio device every sendPingMessageEveryXMs milliseconds to keep the connection alive. Remootio closes the connection if no message is received for 120 seconds. If no message is received from Remootio within (sendPingMessageEveryXMs/2) milliseconds after PING frame is sent the API client considers the connection to be broken and closes it. It's not recommended to set sendPingMessageEveryXMs below 10000 (10 seconds).
     */
    constructor(DeviceIp, ApiSecretKey, ApiAuthKey, sendPingMessageEveryXMs){
        super()
        //Input check
    ...
    connect(autoReconnect){
        ...
        this.websocketClient.on('message', (data)=> {
            try{ //We process the messsage received from the API
                let rcvMsgJson= JSON.parse(data) //It must be JSON format
                ...
                if (rcvMsgJson &&rcvMsgJson.type == "ENCRYPTED"){ //if it's an encrypted frame we decrypt it and then this.emit the event
                    ...
                    if (decryptedPayload != undefined){
                        if (decryptedPayload.challenge != undefined){ //If it's an auth challenge
                        ...
                        if (decryptedPayload.response && decryptedPayload.response.id != undefined){
                            ...
                            if (this.lastActionId < decryptedPayload.response.id    //But we only increment if the response.id is greater than the current counter value
                                || (decryptedPayload.response.id == 0 && this.lastActionId == 0x7FFFFFFF)){ //or when we overflow from 0x7FFFFFFF to 0
                            ...
                            if (decryptedPayload.response.type == "QUERY" && this.waitingForAuthenticationQueryActionResponse == true){
     ...
     */
    sendEncryptedFrame(unencryptedPayload){
        if (this.websocketClient != undefined && this.websocketClient.readyState == WebSocket.OPEN){
            if (this.apiSessionKey != undefined){
                //Upon connecting, send the AUTH frame immediately to authenticate the session
                let encryptedFrame = apicrypto.remootioApiConstructEncrypedFrame(JSON.stringify(unencryptedPayload),this.apiSecretKey,this.apiAuthKey,this.apiSessionKey)
     ...
     */
    sendQuery(){
        this.sendEncryptedFrame({action:{
            type:"QUERY",
    ...
    get isAuthenticated(){
        if (this.websocketClient != undefined && this.websocketClient.readyState == WebSocket.OPEN){
            if (this.apiSessionKey != undefined){ //If the session is authenticated, the apiSessionKey must be defined
                return true;
            }
            else{
                return false;
